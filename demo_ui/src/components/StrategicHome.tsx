import { Calendar, AlertCircle, CheckCircle, Clock, ArrowRight, ChevronRight, Home, Building2, TreePine, Users } from 'lucide-react';
import { Card, CardContent, CardDescription, CardHeader, CardTitle, CardFooter } from "./ui/card";
import { Badge } from "./ui/badge";
import { Button } from "./ui/button";
import { Progress } from "./ui/progress";
import { MapView } from "./views/MapViewInteractive";
    ...
    <div className="space-y-4">
      <Card className="border-l-4 border-l-blue-500 shadow-sm">
        <CardContent className="p-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Housing Target</p>
            <div className="flex items-baseline gap-2 mt-1">
              <h3 className="text-xl font-bold text-slate-900">13,500</h3>
              <span className="text-xs text-emerald-600 font-medium">38% done</span>
            </div>
          </div>
      ...
      <Card className="border-l-4 border-l-purple-500 shadow-sm">
        <CardContent className="p-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Employment</p>
            <div className="flex items-baseline gap-2 mt-1">
              <h3 className="text-xl font-bold text-slate-900">45 ha</h3>
              <span className="text-xs text-slate-500">12ha alloc.</span>
            </div>
          </div>
      ...
      <Card className="border-l-4 border-l-green-500 shadow-sm">
        <CardContent className="p-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Green Belt</p>
            <div className="flex items-baseline gap-2 mt-1">
              <h3 className="text-xl font-bold text-slate-900">22%</h3>
...
export function StrategicHome({ onOpenProject }: StrategicHomeProps) {
          ...
          <div className="relative overflow-hidden rounded-2xl text-white shadow-xl" style={{ backgroundColor: 'var(--color-ink)' }}>
              ...
              <div className="relative p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  <div className="max-w-2xl">
                      ...
                      <h2 className="text-3xl font-bold tracking-tight mb-2">Baseline Stage: In Progress</h2>
                      <p className="text-blue-100/90 text-lg leading-relaxed max-w-xl">
                          You are currently in Month 6 of your 30-month Local Plan journey. Transport evidence is the critical path item this week.
                      </p>
                  </div>
                  <Button 
                  ...
                  >
                      Open Plan Studio <ChevronRight className="ml-2 w-4 h-4" />
                  </Button>
              </div>
          </div>
      </section>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border shadow-sm overflow-hidden" style={{ borderColor: 'var(--color-neutral-300)' }}>
            <CardHeader className="pb-3 border-b border-slate-100 bg-slate-50/50">
              <CardTitle className="text-base font-semibold" style={{ color: 'var(--color-ink)' }}>Strategic Context</CardTitle>
            </CardHeader>
            <div style={{ height: '450px', width: '100%', position: 'relative', backgroundColor: '#f1f5f9' }}>
              <MapView workspace="plan" />
            </div>
          </Card>
