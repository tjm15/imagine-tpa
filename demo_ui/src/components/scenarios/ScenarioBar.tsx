...
export function ScenarioBar({ scenarios, selectedId, onSelect, onCreateNew }: ScenarioBarProps) {
  return (
    <div className="bg-slate-50 border-b border-slate-200 px-6 pt-6 pb-6 flex-shrink-0">
      {/* Section Label */}
      <h2 className="text-xs font-bold text-slate-500 tracking-[0.1em] uppercase mb-4">
        Strategic Scenarios
      </h2>
      ...
      <div className="flex items-stretch gap-3">
        ...
        <button
          onClick={onCreateNew}
          className="flex-shrink-0 w-40 flex flex-col items-center justify-center p-4 rounded-xl border-2 border-dashed border-slate-300 bg-white hover:bg-slate-50 hover:border-indigo-400 hover:text-indigo-600 text-slate-400 transition-all duration-200 gap-2 group min-h-[140px]"
        >
